...
def fixTeam(input):
    return {
        ...
        'GS': 'GSW',
        'SA': 'SAS',
        }.get(input, input)    # returns itself (second argument) if not found in the dictionary
...
def makePrediction(team1,team2):
    ...
    team1OR = team1Rolling.ix[:,-2:-1].values[0][0]
    team1DR = team1Rolling.ix[:,-1:].values[0][0]
    winRate1 = team1Rolling.ix[:,1:2].values[0][0]
    
    team2OR = team2Rolling.ix[:,-2:-1].values[0][0]
    team2DR = team2Rolling.ix[:,-1:].values[0][0]
    winRate2 = team2Rolling.ix[:,1:2].values[0][0]
    
    return (result,team1OR,team1DR,winRate1,team2OR,team2DR,winRate2)

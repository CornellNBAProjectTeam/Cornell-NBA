...
def loadAndSortData(year):
    folder = "Prepared/"
    data = pd.read_csv(folder + year + "_seasonDataSet.csv") #Loads our dataset
...
def trainScoreDifference(years = ['2008','2009','2010','2011','2012','2013','2014','2015']):
            ...
            if not team.empty:
                ...
                for match in range(1,team_rolling.shape[0]): #Note we start at index 1 to ignore the team's first match (rolling average of 1st match is NAN)
                    ...
                    if np.all(opponent_rolling.iloc[0][1:].isnull()):
                        ...
                        predDF.iloc[vals,0:1] = team.iloc[[match]]['tm0_PTS'].values - team.iloc[[match]]['tm1_PTS'].values #team.iloc[[match]]['win'].values
    ...
    print "Accuracy: " + str((1 - scorerPPTS(clf, X_test, y_test)))
    ...
    with open('my_dumped_classifierXXX.pkl', 'wb') as fid:
...
def scorerPTS(estimator, X, y):
    ...
    actual = y
    predicted = estimator.predict(X)[0]
...
def scorerPPTS(estimator, X, y):
    ...
    for i in range(0,X.shape[0]):
        total += scorerPTS(estimator,X.iloc[i:(i+1),:],y[i])
    return (total/X.shape[0])

trainScoreDifference(years = ["2015"])